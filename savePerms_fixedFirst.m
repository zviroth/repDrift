...
function res = savePerms(rois,toZscore,nperms,r2thresh)

if ieNotDefined('rois'), rois = [1]; end%1=GLMdenoise, 2=without GLMdenoise, i.e. betas2.
if ieNotDefined('toZscore'), toZscore = 0; end%1=GLMdenoise, 2=without GLMdenoise, i.e. betas2.
if ieNotDefined('nperms'), nperms = 2; end%1=GLMdenoise, 2=without GLMdenoise, i.e. betas2.
if ieNotDefined('r2thresh'), r2thresh = 0; end%1=GLMdenoise, 2=without GLMdenoise, i.e. betas2.
...
for isub=1:nsubjects
    ...
    for iperm=1:nperms
        permOrders{isub}(iperm,:) = [1 1+randperm(nsessions-1)];
    end
    for iroi=rois%1:figRoi%nrois
        ...
        meanBetas(iroi,isub,1:nsessions) = mean(sessBetas{iroi}(:,goodVox),2);
        meanStdBetas(iroi,isub,1:nsessions) = mean(sessStdBetas{iroi}(:,goodVox),2);
        ...
        for idist=1:nsessions
           temp = squeeze(r2split{iroi}(isub,:,:));%includes NaNs for sessinos that don't exist
            ...
            temp = squeeze(r2oriSplit{iroi}(isub,:,:));
            ...
            temp = squeeze(pearsonR{iroi}(isub,:,:));
            ...
            temp = squeeze(pearsonRori{iroi}(isub,:,:));
...
%             initIndex = 2;%first index is within session
        ...
        for iperm=1:nperms
            ...
            permOrder = [permOrder nsessions+1:maxSessions];%the additional sessions correspond to NaNs
...
save(fullfile(saveFolder, ['perm_fixedFirst_' num2str(nperms) zscoreStr normalizeStr r2threshStr versionStr '.mat']),...
...
res = fullfile(saveFolder, ['perm' num2str(nperms) zscoreStr normalizeStr r2threshStr versionStr '.mat']);