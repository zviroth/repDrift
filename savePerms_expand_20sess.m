...
function res = savePerms_expand_20sess(rois,toZscore,nperms,r2thresh,fixedFirst)
...
load(fullfile(saveFolder,['betasVar.mat']),'roiBetasVar20');
roiBetasVar = roiBetasVar20;

for isub=1:nsubjects
    isub
    subjNum = subjects(isub);
    load(fullfile(saveFolder, ['regressSessCombineROI_sub' num2str(subjNum) zscoreStr '.mat']),'allRoiPrf',...
        'roiNsdCorr','roiNsdOriCorr','roiNsdOriR2','roiNsdR2','roiNsdOriRss',...
        'nsplits',...
...
%             initIndex = 2;%first index is within session
        ...
        autocorrMeanCoef(iroi,isub,:) = autocorr(squeeze(mean(stdCoef{iroi,isub}(:,goodVox),2)),'NumLags',autocorrLength-1);%mean over voxels
        autocorrMeanCoefOri(iroi,isub,:) = autocorr(squeeze(mean(stdOriCoef{iroi,isub}(:,goodVox),2)),'NumLags',autocorrLength-1);%mean over voxels
...
save(fullfile(saveFolder, ['perm_20sess' fixedFirstStr num2str(nperms) zscoreStr normalizeStr r2threshStr  '.mat']),...
    'toNormalize','toZscore', 'useMedian','r2thresh','nrois','rois', ...
    'permOrders', 'subSessions', 'subjects', 'minSessions',...
     'rssOriDistSess','rssOriDist','rssOriSplit','rssOriDistPerm','rssOriDistSessPerm',...
    'r2split','r2oriSplit','pearsonRori','pearsonR',...
    'r2Dist','r2OriDist','pearsonDist','pearsonOriDist','rssOriPerm',...
