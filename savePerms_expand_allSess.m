...
function res = savePerms_expand_allSess(rois,toZscore,nperms,r2thresh,fixedFirst)
...
%     else%randomize for subject 1
        for iperm=1:nperms
            if fixedFirst
                permOrders{isub}(iperm,:) = [1 1+randperm(nsessions-1)];%ONLY PERMUTING FIRST 30 SESSIONS
%                 permOrders{isub}(iperm,:) = [1 1+randperm(minSessions-1)];%ONLY PERMUTING FIRST 30 SESSIONS
            else
                permOrders{isub}(iperm,:) = randperm(nsessions);%ONLY PERMUTING FIRST 30 SESSIONS
...
%     end
    for iroi=rois%1:figRoi%nrois
        ...
        for idist=1:nsessions
           temp = squeeze(r2split{iroi}(isub,1:nsessions,1:nsessions));%includes NaNs for sessinos that don't exist
            ...
            temp = squeeze(r2oriSplit{iroi}(isub,1:nsessions,1:nsessions));
            ...
            temp = squeeze(pearsonR{iroi}(isub,1:nsessions,1:nsessions));
            ...
            temp = squeeze(pearsonRori{iroi}(isub,1:nsessions,1:nsessions));
...
%             lagMatrix = toeplitz(0:-1:1-nsessions,0:nsessions-1);
            for idist=1:nsessions-1
                ...
                r2perm{iroi}(isub,iperm,1:nsessions,1:nsessions) = temp;
                ...
                r2oriPerm{iroi}(isub,iperm,1:nsessions,1:nsessions) = temp;
                ...
                pearsonPerm{iroi}(isub,iperm,1:nsessions,1:nsessions) = temp;
                ...
                pearsonOriPerm{iroi}(isub,iperm,1:nsessions,1:nsessions) = temp;
            end
...
save(fullfile(saveFolder, ['perm_allSess' fixedFirstStr num2str(nperms) zscoreStr normalizeStr r2threshStr  '.mat']),...
...
res = fullfile(saveFolder, ['perm_allSess' fixedFirstStr num2str(nperms) zscoreStr normalizeStr r2threshStr  '.mat']);