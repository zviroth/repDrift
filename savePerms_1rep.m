...
function res = savePerms_1rep(rois,onlyRep,nsessions,toZscore,nperms,r2thresh,fixedFirst)
...
for isub=1:nsubjects
    ...
    load(fullfile(saveFolder, ['regressSessCombineROI_' num2str(onlyRep) 'rep_' num2str(nsessions) 'sess_sub' num2str(isub) zscoreStr '.mat']),'allRoiPrf',...
        'roiNsdCorr','roiNsdOriCorr','roiNsdOriR2','roiNsdR2',...
        'roiNsdOriRss','roiNsdRss','nsplits',...
...
%             initIndex = 2;%first index is within session
        ...
        for ivox=1:size(sessBetas{iroi},2)
            autocorrBetas{iroi,isub}(ivox,:) = autocorr(sessBetas{iroi}(:,ivox),'NumLags',autocorrLength-1);
            autocorrStdBetas{iroi,isub}(ivox,:) = autocorr(sessStdBetas{iroi}(:,ivox),'NumLags',autocorrLength-1);
            autocorrConstant{iroi,isub}(ivox,:) = autocorr(voxConstCoef{iroi}(:,ivox),'NumLags',autocorrLength-1);
            autocorrConstantOri{iroi,isub}(ivox,:) = autocorr(voxConstOriCoef{iroi}(:,ivox),'NumLags',autocorrLength-1);
            autocorrCoef{iroi,isub}(ivox,:) = autocorr(stdCoef{iroi,isub}(:,ivox),'NumLags',autocorrLength-1);
            autocorrCoefOri{iroi,isub}(ivox,:) = autocorr(stdOriCoef{iroi,isub}(:,ivox),'NumLags',autocorrLength-1);
        end
        meanAutocorrBetas(iroi,isub,:) = squeeze(mean(autocorrBetas{iroi,isub}(goodVox,:),1));
        autocorrMeanBetas(iroi,isub,:) = autocorr(squeeze(meanBetas(iroi,isub,1:nsessions)),'NumLags',autocorrLength-1);
        meanAutocorrStdBetas(iroi,isub,:) = squeeze(mean(autocorrStdBetas{iroi,isub}(goodVox,:),1));
        autocorrMeanStdBetas(iroi,isub,:) = autocorr(squeeze(meanStdBetas(iroi,isub,1:nsessions)),'NumLags',autocorrLength-1);
        ...
        autocorrMeanConstant(iroi,isub,:) = autocorr(squeeze(meanConstantCoef(iroi,isub,1:nsessions)),'NumLags',autocorrLength-1);
        autocorrMeanConstantOri(iroi,isub,:) = autocorr(squeeze(meanConstantOriCoef(iroi,isub,1:nsessions)),'NumLags',autocorrLength-1);
        
        autocorrMeanCoef(iroi,isub,:) = autocorr(squeeze(mean(stdCoef{iroi,isub}(:,goodVox),2)),'NumLags',autocorrLength-1);
        autocorrMeanCoefOri(iroi,isub,:) = autocorr(squeeze(mean(stdOriCoef{iroi,isub}(:,goodVox),2)),'NumLags',autocorrLength-1);
...
%                 initIndex = 2;%first index is within session
            ...
            for ivox=1:size(sessBetas{iroi},2)
                autocorrBetasPerm{iroi,isub}(iperm,ivox,:) = autocorr(sessBetas{iroi}(permOrder,ivox),'NumLags',autocorrLength-1);
                autocorrStdBetasPerm{iroi,isub}(iperm,ivox,:) = autocorr(sessStdBetas{iroi}(permOrder,ivox),'NumLags',autocorrLength-1);
                autocorrConstantPerm{iroi,isub}(iperm,ivox,:) = autocorr(voxConstCoef{iroi}(permOrder,ivox),'NumLags',autocorrLength-1);
                autocorrConstantOriPerm{iroi,isub}(iperm,ivox,:) = autocorr(voxConstOriCoef{iroi}(permOrder,ivox),'NumLags',autocorrLength-1);
                autocorrCoefPerm{iroi,isub}(iperm,ivox,:) = autocorr(stdCoef{iroi,isub}(permOrder,ivox),'NumLags',autocorrLength-1);
                autocorrCoefOriPerm{iroi,isub}(iperm,ivox,:) = autocorr(stdOriCoef{iroi,isub}(permOrder,ivox),'NumLags',autocorrLength-1);
            end
            ...
            autocorrMeanBetasPerm(iroi,isub,iperm,:) = autocorr(squeeze(meanBetas(iroi,isub,permOrder)),'NumLags',autocorrLength-1);
            autocorrMeanStdBetasPerm(iroi,isub,iperm,:) = autocorr(squeeze(meanStdBetas(iroi,isub,permOrder)),'NumLags',autocorrLength-1);
            ...
            autocorrMeanConstantPerm(iroi,isub,iperm,:) = autocorr(squeeze(meanConstantCoef(iroi,isub,permOrder)),'NumLags',autocorrLength-1);
            autocorrMeanConstantOriPerm(iroi,isub,iperm,:) = autocorr(squeeze(meanConstantOriCoef(iroi,isub,permOrder)),'NumLags',autocorrLength-1);
            
            autocorrMeanCoefPerm(iroi,isub,iperm,:) = autocorr(squeeze(mean(stdCoef{iroi,isub}(permOrder,goodVox),2)),'NumLags',autocorrLength-1);
            autocorrMeanCoefOriPerm(iroi,isub,iperm,:) = autocorr(squeeze(mean(stdOriCoef{iroi,isub}(permOrder,goodVox),2)),'NumLags',autocorrLength-1);
...
save(fullfile(saveFolder, ['perm_' num2str(onlyRep) 'rep_' num2str(nsessions) 'sess_' fixedFirstStr num2str(nperms) zscoreStr normalizeStr r2threshStr  '.mat']),...
