...
addColorbars = 0;
fixedFirst=0;
toZscore=4;%0=none, 1=zscore, 2=zero mean, 3=normalized std, 4=zero ROI mean
r2thresh = 0;%0; %33;%50;
...
load(fullfile(saveFolder,['betasVar.mat']),'roiBetasVar');
...
for i=1:3
    switch i
     ...
     case 3
            similarityData = rssOriSplit{iroi};
            distSessData = rssOriDistSess;
            distData = rssOriDist{iroi};
            titleStr = 'norm. SS';
            ylabelStr = 'normSS';
            distSessDataPerm = rssOriDistSessPerm;
            distDataPerm = rssOriDistPerm{iroi};
            similarityPerm = rssOriPerm{iroi};
...
% legend off
set(gcf,'position',[250 400 160*cols 800]);
if addColorbars
    set(gcf,'position',[250 300 160*cols 900]);
end
for isubplot=1:rows*cols
    subplot(rows,cols,isubplot)
    legend off
...
if saveFigs
    if ~addColorbars
        savepdf(f,fullfile(figsFolder,['brief_fig1' fixedFirstStr zscoreStr r2threshStr '.pdf']));
    else
        savepdf(f,fullfile(figsFolder,['brief_fig1' fixedFirstStr zscoreStr r2threshStr '_colorbar.pdf']));
    end
end
