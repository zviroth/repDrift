...
% MEAN ROI BETA
% figure
% plot(squeeze(meanBetas(1,:,1:30))')
% hold on
% plot(squeeze(mean(meanBetas(1,:,1:30),2)),'k','linewidth',2);
% title(['r=' num2str(meanBetasCorr,'%4.2f') ' p=' num2str(pPermMeanBetasCorr,'%4.3f')]);
% xlabel('session #');
% ylabel('mean beta');
% 
% [ rho pval] = corr(squeeze(mean(meanBetas(1,:,1:minSessions),2)),(1:minSessions)')
...
% %         histogram(crossCorr{isub,i}(triu(ones(size(crossCorr{isub,i})),1)==1),'DisplayStyle','stairs','NumBins',histBins,'EdgeColor',subColor{isub}); hold on;
%         histogram(crossCorr{isub,i}(triu(ones(size(crossCorr{isub,i})),1)==1),'DisplayStyle','stairs','NumBins',histBins,'EdgeColor',subColor{isub},'Normalization','probability'); hold on;
% %         if isub==1
% %             title(titleStr)
% %         end
%         xlabel('correlation (r)');
...
% legend off
set(gcf,'position',[250 300 160*cols 600]);
if addColorbars
...
% if saveFigs
%     if ~addColorbars
%         savepdf(f,fullfile(figsFolder,['brief_fig1' fixedFirstStr zscoreStr '.pdf']));
%     else
%         savepdf(f,fullfile(figsFolder,['brief_fig1' fixedFirstStr zscoreStr '_colorbar.pdf']));
%     end
% end
...
figure(2)
rows=2;
cols=length(subjects);
for i=1:2
    if i==1
        alldata = squeeze(r2oriSplit{iroi}(:,1:minSessions,1:minSessions));
    else
        alldata=squeeze(pearsonRori{iroi}(:,1:minSessions,1:minSessions));
    end
    tempmax = max(alldata,1);
    tempmin = min(alldata,1);
    for isub=1:length(subjects)
        subplot(rows,cols,isub+(i-1)*cols)
        temp = squeeze(alldata(isub,:,:));
        img=imagesc(temp,'AlphaData',abs(sessDiff)>0); axis square
        caxis([min(temp(abs(sessDiff)>0)), max(temp(abs(sessDiff)>0))]);
        if i==1
            title(['S' num2str(isub)])
        end
%         caxis([min(tempmin(abs(sessDiff)>0)), max(tempmax(abs(sessDiff)>0))]);
...
if saveFigs
    if ~addColorbars
        savepdf(f,fullfile(figsFolder,['brief_fig1_singleSubs' fixedFirstStr zscoreStr '.pdf']));
    else
        savepdf(f,fullfile(figsFolder,['brief_fig1_singleSubs' fixedFirstStr zscoreStr '_colorbar.pdf']));
    end
end