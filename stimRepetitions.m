function res = stimRepetitions(isub,visualRegions,toZscore)
...
sessBetas = betas(visRoiData==iroi,:);
if toZscore==1
    roiBetas{roinum} = [ roiBetas{roinum} zscore(sessBetas,0,2)];
elseif toZscore==2%only remove mean
    roiBetas{roinum} = [ roiBetas{roinum} sessBetas-mean(sessBetas,2)];
elseif toZscore==3%only normalize variance (subtract mean, zscore, add back mean)
    roiBetas{roinum} = [ roiBetas{roinum} mean(sessBetas,2)+zscore(sessBetas-mean(sessBetas,2),0,2)];
elseif toZscore==4%only remove ROI mean
    roiBetas{roinum} = [roiBetas{roinum} sessBetas-mean(mean(sessBetas,2))];
elseif toZscore==0
    roiBetas{roinum} = [ roiBetas{roinum} sessBetas];
end
...
%     end
    ...
    roiBetasVar{visualRegion} = var(combinedRoiBetas{visualRegion},0,2);
end
...
save(fullfile(saveFolder,['stimRepMean_sub' num2str(isub) zscoreStr '.mat']),...
    'visualRegions','combinedRoiBetas','nsess','roiBetasVar');
